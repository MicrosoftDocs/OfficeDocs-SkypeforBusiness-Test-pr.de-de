...
ms:mtpsurl: https://technet.microsoft.com/de-de/library/Gg398562(v=OCS.15)
ms:contentKeyID: 49294434
ms.date: 05/19/2016
...
Enables you to modify configuration properties of one or more servers running the Anwendungsdienst. These servers (also known as Application Servers) host software programs, such as the Anwendung zum Parken von Anrufen, that were developed using the Microsoft Unified Communications Managed-API (UCMA) set. Dieses Cmdlet wurde in Lync Server 2010 eingeführt.
...
The command shown in Example 1 configures the SIP port for the Konferenzankündigungsanwendung on the Application Server ApplicationServer:atl-cs-001.litwareinc.com to 5074.
...
In Example 3, the SIP port for the Konferenzankündigungsanwendung is set to 5074 for all of the Application Servers in the organization. To do this, the command first uses the **Get-CsService** cmdlet to return a collection of all the Application Servers currently in use. This collection is then piped to the **ForEach-Object** cmdlet, which takes each server in the collection, and uses the **Set-CsApplicationServer** cmdlet to set the Konferenzankündigungsanwendung SIP port to 5074.

    Get-CsService -ApplicationServer | ForEach-Object {Set-CsApplicationServer -Identity $_.Identity -CasSipPort 5074} 

## Detailed Description

The Anwendungsdienst hosts a number of Lync Server programs that are not part of the core server components; these programs include the Reaktionsgruppenanwendung, the Konferenzzentrale, and the Konferenzankündigungsanwendung. The Anwendungsdienst takes these programs and fully integrates them into the Lync Server environment.

The **Set-CsApplicationServer** cmdlet enables administrators to modify the configuration settings for any (or all) of the Application Servers deployed in their organization. For example, you can modify the ports used for audio, video, or application sharing traffic, or assign new values to ports used by individual applications such as the Konferenzzentrale or the Konferenzankündigungsanwendung. Note that any time you change ports you will then need to restart the corresponding service.
...
<td><p>Service location of the Anwendungsdatenbank. For example: -ApplicationDatabase &quot;ApplicationDatabase:atl-cs-001.litwareinc.com&quot;.</p></td>
</tr>
<tr class="even">
...
<td><p>Optional</p></td>
<td><p>System.UInt16</p></td>
<td><p>SIP port used by the Konferenzzentrale, used when connecting users to a dial-in conference.</p></td>
</tr>
<tr class="even">
<td><p><em>CasSipPort</em></p></td>
<td><p>Optional</p></td>
<td><p>System.UInt16</p></td>
<td><p>SIP port used by the Konferenzankündigungsanwendung, used to play announcements (for example, &quot;Ken Myer is now exiting&quot;) during a conference.</p></td>
</tr>
<tr class="odd">
...
<td><p>Optional</p></td>
<td><p>System.UInt16</p></td>
<td><p>SIP port used by the Reaktionsgruppenanwendung. The Reaktionsgruppenanwendung provides a way to direct incoming phone calls to a specific group of people, such as an organization’s support team.</p></td>
</tr>
<tr class="odd">
<td><p><em>RgsWcfMtlsPort</em></p></td>
<td><p>Optional</p></td>
<td><p>System.UInt16</p></td>
<td><p>Port used for Windows Communication Foundation (WCF) mutual TLS (MTLS) traffic used by the Reaktionsgruppenanwendung.</p></td>
</tr>
<tr class="even">
